...
shoot = keyboard_check_pressed(vk_rcontrol);
...
//shooting
if(shoot) {
		instance_create_layer(x, y + 8, "Players", oBullet);
		screenShake(3, 2);
}
...
if(!place_meeting(x, y + 1, oWall) || dead) {
	sprite_index = sLaciShotgunSimple;
}
else if(hsp != 0) {
	sprite_index = sLaciShotgunWalk;
}
else sprite_index = sLaciShotgunIdle;
if(hsp != 0) {
